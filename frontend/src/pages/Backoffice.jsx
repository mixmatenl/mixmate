...
function PinManager() {
  ...
  const inp = "w-full border border-white/25 rounded-xl px-4 py-3 bg-white/8 text-white text-base focus:outline-none focus:border-white/50 placeholder-white/35"

  return (
    <form onSubmit={handleSubmit} className="space-y-5 max-w-sm">
      <h3 className="text-white font-bold text-lg">Bartender PIN wijzigen</h3>
      <div>
        <label className="text-white/75 text-sm mb-2 block font-medium">Admin PIN (verificatie)</label>
        <input type="password" value={adminPin} onChange={e => setAdminPin(e.target.value)} placeholder="••••" className={inp} />
      </div>
      <div>
        <label className="text-white/75 text-sm mb-2 block font-medium">Nieuwe bartender PIN</label>
        <input type="password" value={newPin} onChange={e => setNewPin(e.target.value)} placeholder="••••" className={inp} />
      </div>
      <div>
        <label className="text-white/75 text-sm mb-2 block font-medium">Bevestig nieuwe PIN</label>
...
function PumpRow({ pump, ingredients, onUpdate, onDelete, onCalibrate }) {
  ...
  const inp = "w-full border border-white/25 rounded-lg px-3 py-2.5 bg-white/8 text-white text-sm focus:outline-none focus:border-white/45"
    ...
    <div className="bg-white/8 border border-white/15 rounded-2xl overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 cursor-pointer hover:bg-white/5 transition-colors" onClick={() => setOpen(!open)}>
        <div className="flex items-center gap-3">
          <span className="text-white/55 font-mono text-sm w-7 font-semibold">#{pump.slot}</span>
          <span className="text-white text-base font-semibold">
            {pump.ingredient?.name ?? <span className="text-white/45 italic font-normal text-sm">Niet toegewezen</span>}
          </span>
          <span className="text-white/55 text-sm">GPIO {pump.gpio_pin} · {pump.pump_type} · {pump.ml_per_second}ml/s</span>
        </div>
        <svg className={`w-5 h-5 text-white/45 transition-transform ${open ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
        </svg>
      </div>
      {open && (
        <div className="px-5 pb-5 border-t border-white/10 pt-4 grid grid-cols-2 gap-3">
          <div><label className="text-white/75 text-sm mb-2 block font-medium">Slot</label>
            <input type="number" value={form.slot} onChange={e => change('slot', e.target.value)} className={inp} /></div>
          <div><label className="text-white/75 text-sm mb-2 block font-medium">GPIO Pin</label>
          ...
          <div><label className="text-white/75 text-sm mb-2 block font-medium">Type</label>
          ...
          <div><label className="text-white/75 text-sm mb-2 block font-medium">ml/sec</label>
            <input type="number" step="0.1" value={form.ml_per_second} onChange={e => change('ml_per_second', e.target.value)} className={inp} /></div>
          <div className="col-span-2"><label className="text-white/75 text-sm mb-2 block font-medium">Ingrediënt</label>
          ...
          <div className="col-span-2 flex items-center justify-between pt-1">
            <label className="flex items-center gap-2.5 cursor-pointer" onClick={() => change('enabled', !form.enabled)}>
              ...
              <span className="text-white/75 text-sm font-medium">Ingeschakeld</span>
            </label>
            <div className="flex gap-2">
              <button onClick={() => onCalibrate(pump)} className="text-sm px-3 py-2 border border-white/20 rounded-lg text-white/65 hover:border-white/40 hover:text-white transition-all">Kalibreer</button>
...
function PumpsManager({ ingredients }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {adding && (
        <form onSubmit={handleAdd} className="bg-white/8 border border-white/15 rounded-xl p-4 grid grid-cols-2 gap-3">
          {[['Slot','slot','number'],['ml/sec','ml_per_second','number']].map(([l,k,t]) => (
            <div key={k}><label className="text-white/75 text-sm mb-2 block font-medium">{l}</label>
      ...
      {pumps.length === 0 && !adding && <p className="text-white/55 text-base text-center py-8">Geen pompen geconfigureerd.</p>}
    ...
    <div className="min-h-screen bg-[#0d0d0d] flex flex-col">
      <header className="border-b border-white/10 px-6 h-14 flex items-center justify-between">
        <div className="flex items-center gap-6">
          <span className="text-white/85 text-sm tracking-widest uppercase font-bold">Backoffice</span>
        ...
        <div className="flex gap-4">
          <button onClick={lock} className="text-sm text-white/55 hover:text-white/85 transition-colors font-medium">Vergrendel</button>
          <button onClick={onClose} className="text-sm text-white/55 hover:text-white/85 transition-colors font-medium">← Terug</button>
        </div>
      </header>
